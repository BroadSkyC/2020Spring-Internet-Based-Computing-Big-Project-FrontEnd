...
Page({
 ...
 chooseVideo: function() {
    ...
    success: function (res) {
      ...
      var tempFilePaths = res.tempFilePath;
        ...
        uploadVid(res.tempFilePath, 'videos/',
          function (result) {
            console.log("======上传成功视频地址为：", result+"(实际的url中，'.com'后面有个/)");
            that.setData({
              VideoOk:true
            })
            videoUrl=result;
      ...
      success: function (res) {
        ...
        for (var i = 0; i < res.tempFilePaths.length; i++) {
          ...
          uploadImg(res.tempFilePaths[i], 'images/',
            function (result) {
              console.log("======上传成功图片地址为：", result+"(实际的url中，'.com'后面有个/)");
              //这个result就是返给你上传到oss上的地址链接
              coverUrl=result
              wx.hideLoading();
              isImage=true
  ...
  _success() {
    ...
    this.modal.hidePopup();
    wx.request({
      url: app.globalData.baseUrl+'/api/video/getVideoByVideoUrl',
      method:'GET',
      data:{
        videoUrl:videoUrl
      },
